...
Значения для данных параметров могуть быть предзаданы Администратором в конфигурационном файле (см. раздел :ref:`AccountTags_RU` в руководстве по конфигурациям). Они будут использоваться по умолчанию для настраиваемых вычислительных предложений.

Также, в конфигурационном файле (см. раздел :ref:`DefaultServiceOffering_RU`) Администратором могут быть заданы лимиты для параметров предложений (см. раздел :ref:`DefaultServiceOffering_RU`). Т.е. при определении значений параметров предложений нельзя будет указать значения, превышающие заданные лимиты.
...
-  по WebShell, если у ВМ есть тег ``csui.vm.auth-mode`` со значением SSH.  Подробнее о доступе к ВМ через WebShell  в разделе :ref:`WebShell`.  Подробные инструкции по установке плагина WebShell см. в разделе :ref:`WebShellDeployment`.
