...
- Тип - показывает, в какой части системы произошло данное событие, например: ``USER.LOGIN`` - для событий, произошедших при авторизации пользователя в системе; ``VOLUME.RESIZE`` - для события изменения размера диска. 
